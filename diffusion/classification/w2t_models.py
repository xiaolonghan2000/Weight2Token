...
class SimpleProjector(nn.Module):
    """
    u:[B,r,d_out], v:[B,r,d_in], s:[B,r] or [B,r,1] -> x:[B,r,H]
    """
    def __init__(self, d_out: int, d_in: int, hidden_dim: int, dropout: float = 0.1):
        super().__init__()
        self.proj_u = nn.Linear(d_out, hidden_dim, bias=True)
        self.proj_v = nn.Linear(d_in,  hidden_dim, bias=True)
        self.fuse   = nn.Linear(hidden_dim * 2, hidden_dim, bias=True)
        ...
        # sigma gate (weak at start)
        self.sigma_mlp = nn.Sequential(
            nn.Linear(1, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim * 2),
        )
        self.film_eps = nn.Parameter(torch.tensor(0.0))  # starts near identity

    @staticmethod
    def _sigma_norm(s: torch.Tensor) -> torch.Tensor:
        # -> [B,r,1]
        if s.dim() == 2:
            s = s.unsqueeze(-1)
        s = torch.log1p(torch.clamp(s, min=0.0))
        # rank-wise normalize (shape)
        s = (s - s.mean(dim=1, keepdim=True)) / (s.std(dim=1, keepdim=True) + 1e-6)
        return s

    def forward(self, u: torch.Tensor, v: torch.Tensor, s: torch.Tensor) -> torch.Tensor:
        s_n = self._sigma_norm(s)                      # [B,r,1]
        hu = F.gelu(self.ln_u(self.proj_u(u)))         # [B,r,H]
        hv = F.gelu(self.ln_v(self.proj_v(v)))         # [B,r,H]
        x  = self.fuse(torch.cat([hu, hv], dim=-1))    # [B,r,H]

        scale, shift = self.sigma_mlp(s_n).chunk(2, dim=-1)
        eps = self.film_eps
        x = x * (1.0 + eps * torch.tanh(scale)) + eps * shift
...
class SimpleRankPool(nn.Module):
    """
    Multi-query pooling. mask: True=pad
    """
    def __init__(self, hidden_dim: int, num_queries: int = 4, dropout: float = 0.1, sigma_prior_alpha: float = 0.5):
        super().__init__()
        self.hidden_dim = hidden_dim
    ...
    @staticmethod
    def _sigma_feat(s: torch.Tensor) -> torch.Tensor:
        if s.dim() == 3:
            s = s.squeeze(-1)
        s = torch.log1p(torch.clamp(s, min=0.0))
        return s  # [B,r]
...
class SimpleLoRATransformer(nn.Module):
    """
    proj per position -> rank pool -> add layer/module emb -> layer transformer -> pos pool -> MLP head
    """
    def __init__(
        self,
        input_dims, layer_ids, module_ids,
        num_layers: int, num_modules: int,
        hidden_dim: int = 256,
        out_dim: int = 40,
        num_layer_layers: int = 2,
        nhead: int = 8,
        ...
        mlp_dim: int = 256,
        num_queries: int = 4,
        sigma_prior_alpha: float = 0.5,
    ):
        super().__init__()
        assert len(input_dims) == len(layer_ids) == len(module_ids)
        self.num_pos = len(input_dims)

        self.projectors = nn.ModuleList([
            SimpleProjector(d_out, d_in, hidden_dim, dropout) for (d_out, d_in) in input_dims
        ])

        self.rank_pool = SimpleRankPool(hidden_dim, num_queries=num_queries, dropout=dropout, sigma_prior_alpha=sigma_prior_alpha)
        ...
        self.layer_encoder = nn.TransformerEncoder(enc_layer, num_layers=num_layer_layers)
        self.pos_pool = SimpleAttentionPool(hidden_dim, dropout)
    ...
    def forward(self, data, src_key_padding_masks=None):
        ...
        pos_tokens = []
        for p, (u, v, s) in enumerate(x_list):
            rank_mask = src_key_padding_masks[p].bool() if src_key_padding_masks is not None else None
            h = self.projectors[p](u, v, s)                               # [B,r,H]
            T_p = self.rank_pool(h, sigma=s, mask=rank_mask)              # [B,H]
            pos_tokens.append(T_p)

        H_pos = torch.stack(pos_tokens, dim=1)                            # [B,P,H]
        E = self.layer_embedding(self.pos_layer_ids) + self.module_embedding(self.pos_module_ids)  # [P,H]
        H_pos = self.pos_token_norm(H_pos + E.unsqueeze(0))

        # optional: if you have pos_mask and it denotes padding, pass it in here
        pos_mask = data.get("pos_mask", None) if isinstance(data, dict) else None
        if pos_mask is not None:
            pos_kpm = pos_mask.bool()
            H_pos = self.layer_encoder(H_pos, src_key_padding_mask=pos_kpm)
        else:
            H_pos = self.layer_encoder(H_pos)

        pooled = self.pos_pool(H_pos, mask=pos_mask.bool() if pos_mask is not None else None)
        return self.classifier(pooled)
