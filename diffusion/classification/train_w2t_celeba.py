...
def build_label_map(labels_df: pd.DataFrame, attr_cols: list[str]) -> dict[str, torch.Tensor]:
    label_map = {}
...
class CelebALoRADataset(torch.utils.data.Dataset):
    def __init__(self, lora_root: Path, label_map: dict[str, torch.Tensor], names: list[str]):
    ...
    def __getitem__(self, idx: int):
        ...
        celeb_key = celeb_key_from_model_name(model_name)
        ...
        return {"features": features, "meta": meta}, self.label_map[celeb_key]
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Train and evaluate W2T on CelebA-LoRA.")
    ap.add_argument("--lora_root", type=str, default="./data_generation/outputs/celeba/models_rank8_full")
    ap.add_argument("--labels_csv", type=str, default="./data_prepare/outputs/celeba_labels.csv")
    ap.add_argument("--split_dir", type=str, default="./data_prepare/outputs/splits_celeba_rank8_full")
    ap.add_argument("--cache_path", type=str, default="")
    ap.add_argument("--checkpoint_dir", type=str, default="./classification/checkpoints")
    ...
    ap.add_argument("--wandb_project", type=str, default="W2T-CelebA")
    args = ap.parse_args()
    ...
    if cache_path and cache_path.exists():
        ...
        train_set = CelebALoRADataset(lora_root, label_map, train_names)
        valid_set = CelebALoRADataset(lora_root, label_map, valid_names)
        test_set = CelebALoRADataset(lora_root, label_map, test_names)
